...
`LCTL(kc)`,`LSFT(kc)` etc :Modとキー同時押し
```
[ 8bit ][ 8bit ]
000              <--          3bit
   ^^^^^         <-- Modifier 5bit
...
`LT(layer, kc)`: Tapでキー,HoldでLayer。他のレイヤ関連キーは5bitつまり32レイヤまでいけるがこれは4bitなので16レイヤまでしか指定できない。
```
[ 8bit ][ 8bit ]
0100             <--          4bit
    ^^^^         <-- Layer    4bit
...
`LM(layer, mod)`: Hold時に`MO`とmod送信を同時にやる。これも16レイヤまで。
```
[ 8bit ][ 8bit ]
0101000          <--          7bit
       ^^^^      <-- Layer    4bit
